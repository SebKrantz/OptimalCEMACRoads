...
edges = CSV.read("data/transport_network/csv/graph_orig.csv", DataFrame)
edges = edges[edges.speed_kmh .> 10, :] # Remove ferry connection
...
# Now setting budget
K = (K_base + 1e3) * 2 # 1 or 2 billion USD'15 investment volume (*2 because symmetric)
...
param = init_parameters(annealing = false, labor_mobility = false, cross_good_congestion = true, duality = false,
...
filename = "20g_1b_fixed_cgc_sigma38_rho2_julia" # adjust if sigma != 1.5
...
res_nodes |> CSV.write("results/transport_network/GE/nodes_results_$(filename).csv")
...
res_edges |> CSV.write("results/transport_network/GE/edges_results_$(filename).csv")
