...
edges = CSV.read("data/transport_network/csv/graph_orig_MACR_90kmh_google.csv", DataFrame)
edges.add .= false
# Read Additional Links 
edges_add = CSV.read("data/transport_network/csv/graph_add_MACR_90kmh_google.csv", DataFrame)
edges_add.duration .= Inf
edges_add.add .= true
# Combining
edges = vcat(select(edges, :add, :from, :to, :distance, :duration, :ug_cost_km => :cost_per_km, :border_dist), 
             select(edges_add, :add, :from, :to, :distance, :duration, :cost_km => :cost_per_km, :border_dist))
...
# Read Nodes Data
nodes = CSV.read("data/transport_network/csv/graph_nodes_MACR_90kmh_google.csv", DataFrame)
nodes.population /= 1000 # Convert to thousands
nodes.outflows /= 1000   # Adjust in line with population
nodes.productivity = nodes.gdp ./ nodes.population ./ 1e6 
...
sum(population .> 2000 .|| nodes.outflows .> 1000) == 6
...
for i in 1:n
    ...
    if with_ports && nodes.outflows[i] > 0
        productivity[i, nodes.product[i]] += (2.1 * nodes.outflows[i]) / population[i] # Productivity of ports
    end
end
...
max_mask = max.(infra_matrix, adj_matrix .* 90) #  
...
for Ki in [1e3, 2e3, 4e3, 5e3] # 3.2e3 # 2327 # [3.2e3] # 
    # Now setting budget
    ...
    for rho in [2]
        ...
        res_nodes |> CSV.write("results/transport_network/GE/nodes_results_20g$(with_ports ? "" : "_noport")_$(Int(Ki))m_fixed_cgc$(gamma == gamma_IRS ? "_irs" : "")_sigma$(sigma)_rho$(rho)_duality_julia_MACR_90kmh_google.csv")
        ...
        res_edges |> CSV.write("results/transport_network/GE/edges_results_20g$(with_ports ? "" : "_noport")_$(Int(Ki))m_fixed_cgc$(gamma == gamma_IRS ? "_irs" : "")_sigma$(sigma)_rho$(rho)_duality_julia_MACR_90kmh_google.csv")
    end
end
