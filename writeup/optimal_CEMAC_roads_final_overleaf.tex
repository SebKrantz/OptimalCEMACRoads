...
\raggedright
\scriptsize 
\emph{Notes:} Figure shows (inverse-distance weighted) production of top 11 crops within 50km radius of each node (LHS) and IWI within 10km around each node times node population (RHS). Both are combined to distribute GDP across nodes. \\ \vspace{-10mm}
\end{figure}
...
Following \citet{krantz2024optimal}, we assume a 48h transit cost (24h per border) for transiting through 3rd countries. %, the focus should not be on the impact of border frictions except to make it clear that they should be removed as much as possible. 
...
\end{equation}
In line with \citet{collier2016cost}'s treatment of continuous variables, we take the natural log of $\Omega_j$, and find that it ranges between 0 and 7.5, with a median of 2.84. Thus, since \citet{collier2016cost}'s dummy coefficient is 0.3, we assign a coefficient of $0.3/2.84\approx 0.1$. Our road cost equation is thus
...
We assume an economy consisting of a set of locations $j \in \mathcal{J}$ connected by a transport network. Preferences of a representative worker in location $j$ are given by a normalized inequality-averse Cobb-Douglas utility function over consumption $c_j$ and housing $h_j$
\begin{equation}
...
Detailed engineering expertise guided the design and planning of CEMAC road and flagship projects. However, these efforts often overlooked critical social and economic dimensions, as well as the potential for projects to generate broader economic benefits. This omission can result in economically suboptimal investments that may fail to achieve financial viability in the medium term. 
To investigate this, the final section of this paper jointly evaluates several major road projects in the region—including key corridors such as the CD13 project—using the economic modeling framework developed herein. This approach enables a comprehensive assessment of market-access gains, welfare impacts, and cost-effectiveness, ensuring that investment decisions align with both engineering design, technical feasibility and economic optimization. \newline 
